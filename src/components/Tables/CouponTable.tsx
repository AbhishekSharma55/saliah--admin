...
export function CouponDataTable({ data }: { data: CouponSchema[] }) {
  const [sorting, setSorting] = useState<SortingState>([]);
  const router = useRouter();
  const { onOpen } = useDeleteCoupon();
  const columns: ColumnDef<CouponSchema>[] = [
    {
      accessorKey: "coupon",
      header: ({ column }) => {
        return (
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <>
          {row.original?.maximum_discount || 0} -{" "}
        </>
      ),
    },
    {
...

    {
      ...
      cell: ({ row }) => {
        ...
        const couponName = row.original.coupon;
              ...
              <DropdownMenuGroup>
                <DropdownMenuItem asChild>
                  <Link
                    ...
                    href={`/admin/dashboard/coupons/coupon?type=edit&_id=${id}`}
                  >
    ...
    <div className=" rounded-md border flex-1 px-10 overflow-y-auto">
      <div className="flex justify-between py-2">
        ...
        <Button asChild size="icon">
          <Link href="/admin/dashboard/coupons/coupon?type=create">
            <PlusSquare />
          </Link>
        </Button>
      </div>
